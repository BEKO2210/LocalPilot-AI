...
export function DashboardMobileNav({ slug, active }: DashboardMobileNavProps) {
  return (
    ...
    >
      <ul className="flex flex-nowrap gap-1.5 overflow-x-auto px-3 py-2 [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          return (
            <li key={item.key} className="flex-shrink-0">
              <Link
                ...
                className={cn(
                  "lp-focus-ring flex h-10 items-center gap-1.5 rounded-full px-4 text-sm font-medium transition-colors",
                  isActive
